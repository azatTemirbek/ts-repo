...
export const capitalizeWords = (s:string) =>
	s.split(SPACE).map((element) => capitalize(element)).join(SPACE);
...
export const transformPipe = (...ops:CallableFunction[]) => ops.reduce((accumulator, element) => _pipe(accumulator, element));
